...
def test_change_tokens_reflect_same_size_overwrite_after_listing(any_fs):
    any_fs.pipe_file("cached", b"old")
    any_fs.ls("")
    old_checksum = any_fs.checksum("cached")
    old_ukey = any_fs.ukey("cached")

    any_fs.pipe_file("cached", b"new")

    assert any_fs.cat_file("cached") == b"new"
    assert any_fs.checksum("cached") != old_checksum
    assert any_fs.ukey("cached") != old_ukey
