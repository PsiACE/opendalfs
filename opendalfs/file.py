...
class OpendalAsyncFileHandle:
    ...
    async def readinto(self, b) -> int:
        ...
        if self.closed:
            raise ValueError("I/O operation on closed file.")
        await self._drain_pending_seek()
        n_raw = await self._maybe_await(self._file.readinto(b))
    ...
    async def readline(self, size: int | None = -1) -> bytes:
        ...
        await self._drain_pending_seek()
        if size is None or size < 0:
            line = await self._maybe_await(self._file.readline())
        else:
            line = await self._maybe_await(self._file.readline(size))
        self._loc += len(line)
        return line
    ...
    def commit(self) -> None:
        raise NotImplementedError("commit is not supported in async mode")

    def discard(self) -> None:
        raise NotImplementedError("discard is not supported in async mode")
